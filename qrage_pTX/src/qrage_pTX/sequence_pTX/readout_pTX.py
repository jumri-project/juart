...
from .sampling_pTX import AngularTrajectory


class ReadoutKernel_pTX:
    """
    ...
    def __init__(
        ...
        self.rf.signal = self.rf.signal.astype(
            np.complex128
        )  # Ensure RF signal is complex for pTx manipulation
        self.rf_ptx = deepcopy(self.rf)
        # repeat signal for each channel
        self.rf_ptx.signal = np.tile(self.rf.signal, (num_tx))
        iSamples_per_channel = self.rf.t.shape[0]
        ...
        self.rf_ptx.t = np.tile(self.rf.t, (num_tx))
    ...
    def run(
        ...
        # copy phase of pulse to pTx version
        self.rf_ptx.phase_offset = self.rf.phase_offset
