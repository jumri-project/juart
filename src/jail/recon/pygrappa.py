...
def pygrappa(
    ...
    lamda: float = 0.01,
    memmap: bool = False,
    memmap_filename: str = "out.memmap",
    silent: bool = True,
):
    ...
    with NTF() as fP, NTF() as fA, NTF() as frecon:
        ...
        if not silent:
            print(
                ("Training and application of weights took %gseconds" % (time() - t0))
            )
        ...
        if memmap:
            fin = np.memmap(
                memmap_filename, dtype=recon.dtype, mode="w+", shape=fin_shape
            )
            fin[:] = np.moveaxis((recon + kspace)[kx2:-kx2, ky2:-ky2, :], -1, coil_axis)
            del fin
            return None
