...
from phantoms import zpad
from phantoms.shimming.SphericalHarmonics import getSphericalHarmonics
from phantoms.shimming.VoxelCoordinates import getVoxelCoordinates
...
def tissue_map(B0):
    ...
    We assume skin, glial matter and meat to have susceptibilities comparable to muscle (0.000, 0.000) and white matter (-0.030).
    """
    ...
    if B0 == 7.0:
        ...
        data = ((0, 1,  0.83, 0.7, 1, 1, 1, 0, 0.86, 0.77),
        ...
        raise NotImplementedError("Parameters only defined for field strenghts of 1.5T, 3.0T and 7.0T")
...
class BrainPhantom5D(object):
    ...
    def __init__(self, B0=1.5, B1=5, TR=30, FM_scaling=1, CS_scaling=0, B0_shimming=True):
        ...
        self.MSS = M0 * np.sin(B1_rad) * (1 - np.exp(-TR * R1)) / (1 - np.cos(B1_rad) * np.exp(-TR * R1))
    ...
    def signal(self, TP, TE, TS, IE, slice_z=None):
        ...
        s = MSS * (1 - (1 + IE) / (1 + IE * np.exp(-TS * R1s)) * np.exp(-TP * R1s)) * np.exp(-TE * R2s)
