...
rf_const = (
    2 * pi / 1000
)  # from Pulseq[Hz]=[1/s] to JEMRIS[rad/ms] rf magnitude conversion constant
g_const = (
    2 * pi / 1e6
)  # from Pulseq [Hz/m] to JEMRIS [(rad/ms)/mm] gradient conversion constant
slew_const = g_const / 1000  # from Pulseq [Hz/(m*s)] to JEMRIS [(rad/ms)/(mm*ms)]
ga_const = (
    2 * pi / 1000
)  # from Pulseq[1/m] to JEMRIS [2*pi/mm] gradient area conversion constant
                ...
                if rf != None:
                    for ch in range(NRFChannels):  # Added: JF
                        ...
                        if NRFChannels > 1:
                            ...
                            RFFormFileName = (
                                os.path.splitext(rf_shapes_path_dict[rf_id])[0]
                                + "_ch"
                                + str(ch)
                                + ".h5"
                            )
            ...
            if key in ["gx", "gy", "gz"]:
                    ...
                    if g.type == "trap":
                        if g.amplitude != 0:
                            ...
                            if g.flat_time > 0:
                                ...
                                g_atom.set(
                                    "MaxAmpl", str(np.absolute(g.amplitude * g_const))
                                )  # limit amplitude
                                g_atom.set(
                                    "Area",
                                    str(
                                        0.5
                                        * (g.rise_time + g.fall_time)
                                        * g.amplitude
                                        * ga_const
                                    ),
                                )
                        ...
                        raise ValueError(
                            "Gradient's type should be either trap or grad"
                        )
                ...
                if exists_adc and not adc_already_added:
                    ...
                    if adc != None:  # Added: JF
                        ...
                        gzero_adc.set(
                            "Axis", "GX"
                        )  # Just set to X; seems not to work otherwise
                        gzero_adc.set("InitialPhase", str(phase_offset))
                        gzero_adc.set(
                            "ADCFlag", str(2)
                        )  # ADCFlag=2 means "this gradient is for ADC"
...
def save_rf_library_info(seq, out_folder):
    ...
    for rf_id in list(seq.rf_library.data.keys()):  # for each RF ID
        ...
        for ch in range(NChannels):
            ...
            times_ch = times[
                ch * len(times) // NChannels : (ch + 1) * len(times) // NChannels
            ]
            magnitude_ch = np.absolute(
                rf.signal[ch * len(times_ch) : (ch + 1) * len(times_ch)]
            )
            phase_ch = np.angle(
                rf.signal[ch * len(times_ch) : (ch + 1) * len(times_ch)]
            )
...
def save_grad_library_info(seq, out_folder):
    ...
    for nb in range(1, len(seq.block_events) + 1):
        ...
        for axis_ind, g_ind in enumerate([gx_ind, gy_ind, gz_ind]):
            ...
            # print("Gradient indices for block ", nb, ": ", gx_ind, gy_ind, gz_ind)
            # print("Processed gradient indices: ", processed_g_inds)
            # print("Gradient library keys: ", list(seq.grad_library.data.keys()))

            if (
                g_ind != 0
                and seq.grad_library.type[g_ind]
                == "g"  # JF: Change to account for types used in recent pulseq format
                ...
                if axis_ind == 0:
                    t_points = this_block.gx.tt  # JF: Changed to use tt instead of t
                    g_shape = this_block.gx.waveform
                elif axis_ind == 1:
                    t_points = this_block.gy.tt  # JF: Changed to use tt instead of t
                    g_shape = this_block.gy.waveform
                elif axis_ind == 2:
                    t_points = this_block.gz.tt  # JF: Changed to use tt instead of t
